...
输入选题  →  内容质检  →  AI检测闭环  →  内容类型分支  ─┬─ A 图文 → 文案+卡片 ──────────→ 多平台分发
                                                       ├─ B 口播 → 剪辑+封面 ──────────→ 多平台分发
...
> ⚠️ 文案完成后，必须跑 **Step 1.6 AI 写作检测闭环**（dbs-ai-check → 修改 → 复检 → 通过），再进入 A.2 生成卡片。不跳过。
...
> ⚠️ 脚本完成后，必须跑 **Step 1.6 AI 写作检测闭环**，再进入 C.1.5 钩子优化和配音。
