...
这个内容可能不适合用一键成片来做。
原因：{具体原因}
建议：{替代方案，如「试试用视频形式」「建议你自己拍照片后我们帮你排版」}
如果你确定要继续，我可以尽力做，但效果可能不如预期。
```

### 路径 B（口播）能力边界

口播 = 对着镜头讲。核心是「人 + 表达」，不是「画面 + 信息」。

**适合口播**：
| 类型 | 为什么 |
|------|--------|
| 观点 / 认知输出 | 一个人的思考，靠表达力撑 |
| 个人经历 / 故事 | 真实感，镜头前聊出来 |
| 避坑 / 经验分享 | 「我踩过的坑」口语感强 |
| 行业观察 / 快评 | 时效性强，不需要复杂画面 |

**不适合口播**：
| 类型 | 原因 | 建议 |
|------|------|------|
| 软件操作教程 | 观众需要看屏幕，不是看你的脸 | → 走 Path C 教学视频 |
| 数据密集内容 | 口播报数字观众记不住 | → 走 Path A 图文 |
| 需要对比/矩阵 | 口播说不清 A vs B 的细节 | → 走 Path A 图文 |
| 无出镜意愿 | 不想露脸或录音 | → 走 Path A 图文 |

### 路径 C（教学视频）能力边界

教学视频 = AI 画面 + 配音。核心是「结构化知识传递」。
...
| 工具操作教程 | 截图/录屏 + 步骤讲解 |
| 概念解释 | 抽象概念用画面可视化 |
| 流程/步骤 | Pipeline 结构天然匹配视频节奏 |
| 科普/知识类 | 信息量适合 1-3 分钟视频 |

**不适合教学视频**：
| 类型 | 原因 | 建议 |
|------|------|------|
| 纯观点/评论 | 不需要画面，口播更有力 | → 走 Path B 口播 |
| 深度长文 | 视频承载不了太密的逻辑 | → 走 Path A 图文 |
| 互动/实操类 | 需要观众跟着做，视频节奏太死 | → 走 Path A 图文 + 分步骤 |
| 高度视觉化内容 | Hyperframes 画面有上限，纯文字视频效果差 | → 考虑 Path A 或 用户自己拍 |
...
| 内容类型 | 推荐主题 |
|---------|---------|
| AI 工具教程 | `bytedance` / `tech-modern` |
| 商业思考 | `bold-navy` / `professional-clean` |
| 生活技巧 | `warm-editorial` / `elegant-rose` |
| 文化人文 | `ink` / `newspaper` |
| 深度长文 | `sspai` / `midnight` |
| 默认通用 | `professional-clean` |
